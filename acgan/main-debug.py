...
parser.add_argument('--dataset', required=True, help='toytestdata')
...
for epoch in range(opt.niter):
    for i, data in enumerate(dataloader, 0):
        ...
        with torch.no_grad():
            # above line is new
            input.resize_as_(real_cpu).copy_(real_cpu) # input.data.resize_as_(real_cpu).copy_(real_cpu)
            dis_label.resize_(batch_size).fill_(real_label)
            # dis_label.data.resize_(batch_size).fill_(real_label)
            aux_label.resize_(batch_size).copy_(label) # aux_label.data.resize_(batch_size).copy_(label)
        ...
        label = np.random.randint(0, num_classes, opt.batchSize)
        noise_ = np.random.normal(0, 1, (batch_size, nz))
        class_onehot = np.zeros((opt.batchSize, num_classes + 1))
        class_onehot[np.arange(opt.batchSize), 0] = label
        
#         eval_onehot = np.zeros((opt.batchSize, num_classes + 1))
#         eval_onehot[np.arange(opt.batchSize),0] = eval_label
        class_age = np.random.randint(0, MAX_AGE, opt.batchSize)
        print(label.shape, batch_size, opt.batchSize, class_onehot[np.arange(opt.batchSize), 1].shape)
...
#         print(errG.data.shape, errG.data)
        all_loss_G += errG.data # was errG.data[0]
        all_loss_D += errD.data # was errD.data[0]
        all_loss_A += accuracy
